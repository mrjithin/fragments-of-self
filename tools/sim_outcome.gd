...
func _ready() -> void:
	...
	await _play("rowan", {"manager": 10, "iris": 35, "rowan": 80, "june": 25}, false, 5)
	_check("wrong alter: alignment drops (5 -2 trigger -1 stressed -1 bond = 1)",
		GameState.ending_alignment == 1)
	_check("wrong alter: stress spikes and clamps to 100", GameState.alter_stress["rowan"] == 100)
	_check("wrong alter: penalty recorded for day-end (-4)", GameState.last_outcome_penalty == -4)
